...
// Converts the temperature 'T' from scale 'scale' to Fahrenheit
double toFahrenheit( double T, char scale )
{
   double outT;
   ...
   switch( scale ) {
       ...
       case DegC:
       case Degc:
           outT = (double(T * CtoFRatio)) + CtoFOffset;
           break;
       
       case DegK:
       case Degk:
           outT = (CtoFRatio * ( T - (double(CtoKOffset * 10 )))) + CtoFOffset;
           break;
...
double toCelsius( double T, char scale )
{
   ...
   switch( scale ) {
       case DegF:
       case Degf:
           outT = (double(1.0/CtoFRatio)) * (T - CtoFOffset);
           break;
...
double toKelvin( double T, char scale )
{
   ...
   switch( scale ) {
       case DegF:
       case Degf:
           outT = ((double(1.0/CtoFRatio)) * (T - CtoFOffset)) + (double(CtoKOffset * 10));
           break;
